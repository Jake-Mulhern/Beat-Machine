...
active_instruments_list = [1 for _ in range(instruments)]
bpm = 240
...
def play_notes():
    for i in range(len(clicked)):
        if clicked[i][active_beat] == 1 and active_instruments_list[i] == 1:
            if i == 0:
                hi_hat.play()
            if i == 1:
                snare.play()
...
def draw_grid(clicks, beat, active_instruments):
    ...
    kick_text = label_font.render('Bass', True, colors[active_instruments[2]])
    ...
    floor_tom_text = label_font.render('Floor Tom', True, colors[active_instruments[5]])
...
def draw_load_menu(index):
    ...
    return exit_button, loading_btn, delete_btn, loaded_rectangle, loaded_info
...
while run:
    ...
    boxes = draw_grid(clicked, active_beat, active_instruments_list)
    ...
    add_beat_text = medium_font.render('+1', True, white)
    sub_beat_text = medium_font.render('-1', True, white)
    screen.blit(add_beat_text, (820, HEIGHT - 140))
    screen.blit(sub_beat_text, (820, HEIGHT - 90))

    ## Clear Board
    clear_btn = pygame.draw.rect(screen, gray, [1150, HEIGHT - 150, 200, 100], 0, 5)
    ...
    ## Save/Load
    save_btn = pygame.draw.rect(screen, gray, [900, HEIGHT - 150, 200, 48], 0, 5)
    load_btn = pygame.draw.rect(screen, gray, [900, HEIGHT - 100, 200, 48], 0, 5)
    save_text = label_font.render('Save', True, white)
    load_text = label_font.render('Load', True, white)
    screen.blit(save_text, (920, HEIGHT - 143))
    screen.blit(load_text, (920, HEIGHT - 90))

    if save_menu:
        exit_btn, saving_btn, entry_rectangle  = draw_save_menu(beat_name, typing)
    if load_menu:
